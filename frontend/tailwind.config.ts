...
const config: Config = {
  darkMode: 'class',
  /*
    Один глоб на весь src, а не перелік окремих тек.

    Перелік мовчки ламається щоразу, коли зʼявляється новий шар: класи, які
    трапляються деінде в проєкті, генеруються й далі, а унікальні для нових
    файлів — ні. Найгірше видно на довільних значеннях: grid-cols-[1fr_400px]
    просто не існувало в стилях, тож сітка складалась в одну колонку, і
    здавалося, що «поплила» верстка, а не збірка стилів.
  */
