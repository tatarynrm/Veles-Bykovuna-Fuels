  ...
  async getAllTransactions(
    ...
    if (brand !== 'OKKO') {
      ...
      shellTx = rawShell.map(t => ({
        ...
        contract_id: t.AccountNumber || '',
        contract_name: t.AccountName || '',
        client_id: t.VehicleRegistration || '',
        client_name: t.DriverName || '',
        card_num: t.CardPAN,
        azs_name: t.SiteName,
        addr_name: [t.SiteName, t.SiteCountry].filter(Boolean).join(' · '),
        product_id: t.ProductCode,
        product_desc: t.ProductName,
        price: t.UnitPrice,
        // Літри лише для пального: у зборів Quantity — не літри, тож обсяг 0 (UI покаже «—»).
        volume: t.FuelProduct ? t.Quantity : 0,
        amnt_trans: t.GrossAmount,
        amount_discount: 0,
        basket_of_goods: false,
        // Кожен тип операції Shell передається окремо (пальне/мийка/збір/…).
        trans_type: t.TransactionTypeCode,
        trans_type_desc: t.TransactionTypeDescription,
        // Оригінальна валюта інвойсу Shell (суми вже сконвертовані у грн).
        currency_code: t.CurrencyCode,
        source_amount: t.SourceGrossAmount,
        reversal: t.IsReturn,
        is_return: t.IsReturn,
        is_fee: t.IsFee,
        is_fuel: t.FuelProduct,
